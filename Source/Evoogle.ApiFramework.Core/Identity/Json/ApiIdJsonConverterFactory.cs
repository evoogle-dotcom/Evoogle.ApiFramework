...
public sealed class ApiIdJsonConverterFactory : JsonConverterFactory
{
    private readonly ApiIdJsonShape _shape;
    public ApiIdJsonConverterFactory(ApiIdJsonShape shape = ApiIdJsonShape.Auto) => _shape = shape;
    ...
    private sealed class ScalarApiIdConverter : JsonConverter<ApiId>
    {
        ...
        public override void Write(Utf8JsonWriter writer, ApiId value, JsonSerializerOptions options)
        {
            ...
            {
                var proxy = new ApiIdJsonConverter();
                proxy.Write(writer, value, options); return;
            }
            var s = value.ToString();
            if (s is null) { writer.WriteNullValue(); return; }
